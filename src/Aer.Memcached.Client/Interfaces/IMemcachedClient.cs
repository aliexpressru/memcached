...
public interface IMemcachedClient
{
    ...
    Task<MemcachedClientResult> StoreAsync<T>(string key, T value, TimeSpan? expirationTime, CancellationToken token, StoreMode storeMode = StoreMode.Set);
    ...
    /// <param name="token">Cancellation token</param>
    /// <param name="storeMode">Store mode</param>
    Task MultiStoreAsync<T>(Dictionary<string, T> keyValues, TimeSpan? expirationTime, CancellationToken token, StoreMode storeMode = StoreMode.Set);
    ...
    Task<IDictionary<string, T>> MultiGetAsync<T>(IEnumerable<string> keys, CancellationToken token);
