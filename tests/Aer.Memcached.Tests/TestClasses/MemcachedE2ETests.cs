...
public class MemcachedE2ETests
{
    ...
    [TestMethodWithIgnoreIfSupport]
    [IgnoreIf(nameof(IsWindows))]
    public async Task WepApi_E2E_Delete_NonExistentKeys_Success()
    {
        var port1 = GeneratePort();
        var port2 = GeneratePort();
        ...
        var result = await client1.Delete(new DeleteRequest
        {
            Key = keyValues.Keys.First()
        });
        
        result.SyncSuccess.Should().BeTrue();
        result.Success.Should().BeTrue();
        
        result = await client1.Delete(new DeleteRequest
        {
            Key = keyValues.Keys.First()
        });
