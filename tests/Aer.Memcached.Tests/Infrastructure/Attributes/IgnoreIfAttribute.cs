...
namespace Aer.Memcached.Tests.Infrastructure.Attributes;
...
internal class IgnoreIfAttribute : Attribute
{
	...
	internal bool ShouldIgnore(ITestMethod testMethod)
	{
		...
		{
			var message =
				$"Conditional ignore method {IgnoreCriteriaMethodName} not found. Ensure the method is in the same class as the test method, marked as `static`, returns a `bool`, and doesn't accept any parameters.";
